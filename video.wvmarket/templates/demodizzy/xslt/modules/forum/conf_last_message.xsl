...
<xsl:stylesheet	version="1.0"
	...
	<xsl:template match="udata[@module = 'forum'][@method = 'conf_last_message']">
		<a href="{@link}" umi:empty="&comment-title;" umi:field-name="name" umi:element-id="{@id}"><xsl:value-of select="@name" /></a>
	...
	<xsl:template match="udata" mode="last_message">
		...
		(<span umi:empty="&forum-message-date;" umi:field-name="publish_time" umi:element-id="{page/@id}">
