					...
					<div class="c">
						<div class="content">
							...
							<xsl:choose>
								...
								<xsl:otherwise>
									<a class="o_button" href="{@link}"><span class="l" /><span class="c">&download;</span><span class="r" /></a>
									...
									<xsl:choose>
										<xsl:when test="$file/@size &gt; '1073741824'">
											<xsl:value-of select="format-number($file/@size div '1073741824', '#.##')" /> Gb)
										</xsl:when>
										<xsl:when test="$file/@size &gt; '1048576'">
											<xsl:value-of select="format-number($file/@size div '1048576', '#.##')" /> Mb)
										</xsl:when>
										<xsl:when test="$file/@size &gt; '1024'">
											<xsl:value-of select="format-number($file/@size div '1024', '#.##')" /> Kb)
										</xsl:when>
										<xsl:otherwise>
											<xsl:value-of select="format-number($file/@size, '#.##')" /> b)
										</xsl:otherwise>
									</xsl:choose>
								</xsl:otherwise>
							</xsl:choose>
